...
InfiniteOS.img: kernel.bin
	dd if=/dev/zero of=InfiniteOS.img bs=512 count=2880 status=none
	dd if=kernel.bin of=InfiniteOS.img conv=notrunc status=none
	printf '\x55\xAA' | dd of=InfiniteOS.img bs=1 seek=510 conv=notrunc status=none
...
clean:
	rm -f *.o *.elf *.bin InfiniteOS.img boot/*.o src/*/*.o
